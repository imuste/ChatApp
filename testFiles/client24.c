...
void writeChatToClient21(int clientSD);
...
{       
        ...
        //read CHAT message from client21 from socket
        readChat(clientSD);
        // printf("Client24 read from client21\n");
        sleep(1);

        //read CHAT message from client22 from socket
        readChat(clientSD);
        // printf("Client24 read from client22\n");
        sleep(1);

        //write the CHAT message into the socket
        writeChatToClient22(clientSD);
        // printf("Client24 wrote to client22\n");

        //write the CHAT message into the socket
        writeChatToClient21(clientSD);
        // printf("Client24 wrote to client21\n");

        //read CHAT message from client 23 from socket
        readChat(clientSD);
        // printf("Client24 read from client23\n");

        //write the CHAT message into the socket
        writeChatToClient23(clientSD);
        // printf("Client24 wrote to client23\n");
...
void writeHello(int clientSD)
{
        ...
        strcpy(header1.msgSource, "client24");
...
void writeChatToClient21(int clientSD)
{
        ...
        strcpy(header2.msgSource, "client24");
        header2.msgSource[sizeof(header2.msgSource) - 1] = '\0';
        strcpy(header2.msgDest, "client21");
        header2.msgDest[sizeof(header2.msgDest) - 1] = '\0';
        header2.contentLength = htonl(52);
        header2.msgID = htonl(2);

        int msgSize = 52;
        char *chat = malloc(msgSize);
        strcpy(chat, "Thank you for your message client 21, from client24!");
...
void writeChatToClient23(int clientSD)
{
        ...
        header2.msgDest[sizeof(header2.msgDest) - 1] = '\0';
        header2.contentLength = htonl(43);
        header2.msgID = htonl(3);

        int msgSize = 43;
        char *chat = malloc(msgSize);
        strcpy(chat, "Nice to meet you client23, this is client24");
...
void writeExit(int clientSD)
{
        ...
        strcpy(header3.msgSource, "client24");
