...
void writeHelloExit(int clientSD);
...
{       
        ...
        //write the HELLO and EXIT message into the socket
        writeHelloExit(clientSD);
...
void writeHelloExit(int clientSD)
{
        ...
        strcpy(header1.msgSource, "client4");
        ...
        strcpy(header2.msgSource, "client4");
        ...
        char *bothHeaders = malloc(100);
        memcpy(bothHeaders, &header1, 50);
        memcpy(bothHeaders + 50, &header2, 50);
        int returnVal = write(clientSD, bothHeaders, 100);
        assert(returnVal != -1);
}
