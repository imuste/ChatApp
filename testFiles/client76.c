...
{       
        ...
        //write the next CHAT message into the socket
        writeChat2(clientSD);

        //read final CHAT message from socket
        readChat(clientSD);
...
void writeHello(int clientSD)
{
        ...
        strcpy(header1.msgSource, "client76");
...
void writeChat1(int clientSD)
{
        ...
        strcpy(header2.msgSource, "client76");
        header2.msgSource[sizeof(header2.msgSource) - 1] = '\0';
        strcpy(header2.msgDest, "client75");
        header2.msgDest[sizeof(header2.msgDest) - 1] = '\0';
        header2.contentLength = htonl(58);
        header2.msgID = htonl(1);

        int msgSize = 58;
        char *chat = malloc(msgSize);
        strcpy(chat, "Hello, how are you doing, this is a message from client76.");
        ...
        int returnVal = write(clientSD, totalmessage, 49);
        assert(returnVal != -1);
        sleep(1);
        returnVal = write(clientSD, totalmessage + 49, 58);
        assert(returnVal != -1);
        sleep(1);
        returnVal = write(clientSD, totalmessage + 107, totalSize - 107);
        assert(returnVal != -1);
        // sleep(1);
}
...
void writeChat2(int clientSD)
{
        ...
        strcpy(header2.msgDest, "client75");
        header2.msgDest[sizeof(header2.msgDest) - 1] = '\0';
        header2.contentLength = htonl(36);
        header2.msgID = htonl(2);

        int msgSize = 36;
        char *chat = malloc(msgSize);
        strcpy(chat, "I'm happy to hear you're doing well.");
        ...
        int returnVal = write(clientSD, totalmessage, 37);
        assert(returnVal != -1);
        sleep(1);
        returnVal = write(clientSD, totalmessage + 37, 37);
        assert(returnVal != -1);
        sleep(1);
        returnVal = write(clientSD, totalmessage + 74, totalSize - 74);
        assert(returnVal != -1);
}
...
void writeExit(int clientSD)
{
        ...
        strcpy(header3.msgSource, "client76");
