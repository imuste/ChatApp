...
void writeHello(int clientSD)
{
        ...
        strcpy(header1.msgSource, "client75");
...
void writeChat1(int clientSD)
{
        ...
        strcpy(header2.msgSource, "client75");
        header2.msgSource[sizeof(header2.msgSource) - 1] = '\0';
        strcpy(header2.msgDest, "client76");
        ...
        int returnVal = write(clientSD, totalmessage, 17);
        assert(returnVal != -1);
        sleep(1);
        returnVal = write(clientSD, totalmessage + 17, 34);
        assert(returnVal != -1);
        sleep(1);
        ...
        strcpy(header3.msgSource, "client75");
        header3.msgSource[sizeof(header3.msgSource) - 1] = '\0';
        strcpy(header3.msgDest, "client76");
        header3.msgDest[sizeof(header3.msgDest) - 1] = '\0';
        header3.contentLength = htonl(8);
        header3.msgID = htonl(2);

        int msgSize1 = 8;
        char *chat1 = malloc(msgSize1);
        strcpy(chat1, "Goodbye!");

        //totalSize = 58
        int totalSize1 = msgSize1 + sizeof(msgHeader);
        char *totalmessage1 = malloc(totalSize1);
        memcpy(totalmessage1, &header3, sizeof(header3));
        memcpy(totalmessage1 + sizeof(header3), chat1, msgSize1);

        char *combined = malloc(21);
        memcpy(combined, totalmessage + 51, totalSize - 51);
        memcpy(combined + 18, totalmessage1, 3);
        returnVal = write(clientSD, combined, 21);

        // returnVal = write(clientSD, totalmessage + 51, totalSize - 51);
        // assert(returnVal != -1);

        // int returnVal = write(clientSD, totalmessage1, 3);
        // assert(returnVal != -1);


        sleep(1);
        returnVal = write(clientSD, totalmessage1 + 3, 40);
        assert(returnVal != -1);
        sleep(1);
        returnVal = write(clientSD, totalmessage1 + 43, totalSize1 - 43);
        assert(returnVal != -1);
}
