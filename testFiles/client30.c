...
{       
        ...
        //to client29 who is not connected yet
        // writeChat1(clientSD);

        //read ERROR msg from socket
        // readError(clientSD);

        sleep(10);

        //write CHAT message to the socket
        //to client29 who is now connected
        writeChat1(clientSD);
...
void writeHello(int clientSD)
{
        ...
        strcpy(header1.msgSource, "client30");
...
void writeChat1(int clientSD)
{
        ...
        strcpy(header2.msgSource, "client30");
        header2.msgSource[sizeof(header2.msgSource) - 1] = '\0';
        strcpy(header2.msgDest, "client29");
        header2.msgDest[sizeof(header2.msgDest) - 1] = '\0';
        header2.contentLength = htonl(40);
        header2.msgID = htonl(4);

        int msgSize = 40;
        char *chat = malloc(msgSize);
        strcpy(chat, "Hello client29 I'm trying to reach you!!");
...
void writeExit(int clientSD)
{
        ...
        strcpy(header3.msgSource, "client30");
