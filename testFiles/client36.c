...
void writeClientList1(int clientSD);
void writeClientList2(int clientSD);
...
{       
        ...
        //write CLIENT LIST message into the socket
        //writes in 2 parts with 15 seconds sleep in between
        writeClientList1(clientSD);

        //read CLIENT LIST from the socket
        readClientList(clientSD);

        //write CLIENT LIST message into the socket
        //writes in 2 parts with 60 seconds sleep in between, so times out
        writeClientList2(clientSD);
...
void writeHello(int clientSD)
{
        ...
        strcpy(header1.msgSource, "client36");
...
void writeClientList1(int clientSD)
{
        ...
        strcpy(header2.msgSource, "client36");
        ...
        int returnVal = write(clientSD, (char *)&header2, 20);
        assert(returnVal != -1);

        sleep(15);

        returnVal = write(clientSD, (char *)&header2 + 20, 30);
        assert(returnVal != -1);
}
...
void writeClientList2(int clientSD)
{
        ...
        char buffer[50];
        returnVal = read(clientSD, buffer, sizeof(buffer));
        if (returnVal == 0) {
                printf("client was successfully removed\n");
                exit(EXIT_SUCCESS);
        }
...
void writeExit(int clientSD)
{
        ...
        strcpy(header3.msgSource, "client36");
