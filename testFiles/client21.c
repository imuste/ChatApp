...
void writeChatToClient22(int clientSD);
void writeChatToClient23(int clientSD);
void writeChatToClient24(int clientSD);
...
{       
        ...
        //write the CHAT message into the socket
        writeChatToClient22(clientSD);
        // printf("Client21 wrote to client22\n");

        //write the CHAT message into the socket
        writeChatToClient23(clientSD);
        // printf("Client21 wrote to client23\n");

        //write the CHAT message into the socket
        writeChatToClient24(clientSD);
        // printf("Client21 wrote to client24\n");


        //read CHAT message from client22 from socket
        readChat(clientSD);
        // printf("Client21 read from client22\n");

        //read CHAT message from client 24 from socket
        readChat(clientSD);
        // printf("Client21 read from client24\n");

        //read CHAT message from client 23 from socket
        readChat(clientSD);
        // printf("Client21 read from client23\n");
...
void writeHello(int clientSD)
{
        ...
        strcpy(header1.msgSource, "client21");
...
void writeChatToClient22(int clientSD)
{
        ...
        strcpy(header2.msgSource, "client21");
        header2.msgSource[sizeof(header2.msgSource) - 1] = '\0';
        strcpy(header2.msgDest, "client22");
        header2.msgDest[sizeof(header2.msgDest) - 1] = '\0';
        header2.contentLength = htonl(67);
        header2.msgID = htonl(1);

        int msgSize = 67;
        char *chat = malloc(msgSize);
        strcpy(chat, "Hello client22, how are you doing, this is a message from client21.");
...
void writeChatToClient23(int clientSD)
{
        ...
        strcpy(header2.msgSource, "client21");
        header2.msgSource[sizeof(header2.msgSource) - 1] = '\0';
        strcpy(header2.msgDest, "client23");
        header2.msgDest[sizeof(header2.msgDest) - 1] = '\0';
        header2.contentLength = htonl(62);
        header2.msgID = htonl(2);

        int msgSize = 62;
        char *chat = malloc(msgSize);
        strcpy(chat, "Hello client23, this is a message from client21, how are you??");
...
void writeChatToClient24(int clientSD)
{
        ...
        strcpy(header2.msgSource, "client21");
        header2.msgSource[sizeof(header2.msgSource) - 1] = '\0';
        strcpy(header2.msgDest, "client24");
        header2.msgDest[sizeof(header2.msgDest) - 1] = '\0';
        header2.contentLength = htonl(35);
        header2.msgID = htonl(3);

        int msgSize = 35;
        char *chat = malloc(msgSize);
        strcpy(chat, "A message from client21 to client24");
...
void writeExit(int clientSD)
{
        ...
        strcpy(header3.msgSource, "client21");
